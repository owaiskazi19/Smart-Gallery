...
//        imageprocess.Act();
    ...
    private void render() {
        ...
        ArrayList<imageFolder> folds = getPicturePaths();
        System.out.println("hereheerere");
    ...
    private ArrayList<imageFolder> getPicturePaths(){
        ...
        try {
            ...
            do{
                ...
                Log.d("first first",datapath);
                //String folderpaths =  datapath.replace(name,"");
                String folderpaths = datapath.substring(0, datapath.lastIndexOf(folder+"/"));

                folderpaths = folderpaths+folder+"/";
                Log.d("folder folder", folderpaths);
                if (listimage.size() > 0) {
                    ...
                    Log.d("lisimage listimage", String.valueOf(listimage));
                    picPaths = listimage;
                    System.out.println(picPaths);
                }
                if (!picPaths.contains(folderpaths)) {
                    picPaths.add(folderpaths);
                    Log.d("changed picpaths", String.valueOf(picPaths));
                    ...
                    if (listimage.size() == 2) {
                        ...
                        Log.d("hello from datapath", datapath);
                    ...
                    for(int i = 0;i<picFolders.size();i++){
                        if(picFolders.get(i).getPath().equals(folderpaths)){
                            Log.d("picFolders", String.valueOf(picFolders.get(i).getPath()));
                            if (listimage.size() > 0) {
                                datapath = null;
                                datapath = listimage.get(i);
                                list.add(datapath);
                                Log.d("Searched picture path ", datapath);
